...
最近は Claude Code にFlutterアプリを一から作ってもらうことが多いのですが、AIは一つのファイルで完結しない状態管理を苦手としているように感じます。
...
`dependencies`に空配列`[]`を指定した場合、最初に実行された後は二度と呼ばれることがないため、`StatefulWidget`の`initState`メソッドのような使い方が出来ます。

`dependencies`に空配列を指定した場合のクリーンナップはWidgetが使われなくなり破棄される時、つまり`StatefulWidget`の`dispose`メソッドに相当します。
